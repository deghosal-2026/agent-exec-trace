...
class ToolResult:
    ...
    Frozen so results can be replayed/shared safely. ``hit`` carries the matched
    knowledge-base answer, ``account_id`` the subject account, ``ok`` the success
    flag, and ``message`` a human label that shows in traces/tool logs.

    Attributes:
        hit: matched knowledge-base answer text (knowledge-base lookups).
        account_id: account the lookup targeted.
        ok: whether the tool succeeded.
        message: short human-readable result label.
    """
...
def search_kb(query: str) -> ToolResult:
    ...
    regardless of seed casing. A miss returns ``ok=False`` with a clear message.
    """
...
def lookup_account(account_id: str) -> ToolResult:
    ...
    any id in the set returns ``ok=False`` (account not found).
    """
...
def infer_estimated_cost(turns: int, per_turn: float) -> float:
    ...
    seeded step counts. ``round(..., 6)`` keeps floats stable and readable.
    """
