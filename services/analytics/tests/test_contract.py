...
def _detectors(exclude_llm: bool = True) -> list[BaseDetector]:
    ...
    if exclude_llm:
        return [d for d in all_dets if not isinstance(d, (
            SemanticLoopDetector, HallucinationDetector, GoalDriftDetector,
            QualityDegradationDetector, ConfusionPatternDetector,
        ))]
...
def test_loop_detector_fires_on_repeated_tool() -> None:
    op = "execute_tool"
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
...
def test_tool_error_rate_detector_fires_on_errors() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", op, status="error", **{"gen_ai.tool.name": "api_call", "gen_ai.operation.name": op}),
            _sp("3", "1", op, status="error", **{"gen_ai.tool.name": "api_call", "gen_ai.operation.name": op}),
            _sp("4", "1", op, status="error", **{"gen_ai.tool.name": "db_query", "gen_ai.operation.name": op}),
            _sp("5", "1", op, status="error", **{"gen_ai.tool.name": "db_query", "gen_ai.operation.name": op}),
            _sp("6", "1", op, status="ok", **{"gen_ai.tool.name": "http_get", "gen_ai.operation.name": op}),
        ])
    ]
...
def test_specific_tool_error_detector_fires() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", op, status="error", **{"gen_ai.tool.name": "flaky_api", "gen_ai.operation.name": op}),
            _sp("3", "1", op, status="error", **{"gen_ai.tool.name": "flaky_api", "gen_ai.operation.name": op}),
            _sp("4", "1", op, status="error", **{"gen_ai.tool.name": "flaky_api", "gen_ai.operation.name": op}),
            _sp("5", "1", op, status="error", **{"gen_ai.tool.name": "flaky_api", "gen_ai.operation.name": op}),
            _sp("6", "1", op, status="ok", **{"gen_ai.tool.name": "stable", "gen_ai.operation.name": op}),
        ])
    ]
    s = _summary(total_tool_calls=5)
    results = _run_detectors(_detectors(), s, spans)
...
def test_retry_storm_detector_fires_on_repeated_errors() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", op, status="error", **{"gen_ai.tool.name": "retry_me", "gen_ai.operation.name": op}),
            _sp("3", "1", op, status="error", **{"gen_ai.tool.name": "retry_me", "gen_ai.operation.name": op}),
            _sp("4", "1", op, status="error", **{"gen_ai.tool.name": "retry_me", "gen_ai.operation.name": op}),
            _sp("5", "1", op, status="error", **{"gen_ai.tool.name": "retry_me", "gen_ai.operation.name": op}),
            _sp("6", "1", op, status="error", **{"gen_ai.tool.name": "retry_me", "gen_ai.operation.name": op}),
            _sp("7", "1", op, status="error", **{"gen_ai.tool.name": "retry_me", "gen_ai.operation.name": op}),
        ])
    ]
...
def test_recovery_path_detector_fires_after_error_recovery() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", op, status="error", **{"gen_ai.tool.name": "unstable", "gen_ai.operation.name": op}),
            _sp("3", "1", op, status="ok", **{"gen_ai.tool.name": "fix_1", "gen_ai.operation.name": op}),
            _sp("4", "1", op, status="ok", **{"gen_ai.tool.name": "fix_2", "gen_ai.operation.name": op}),
            _sp("5", "1", op, status="ok", **{"gen_ai.tool.name": "fix_3", "gen_ai.operation.name": op}),
            _sp("6", "1", op, status="ok", **{"gen_ai.tool.name": "fix_4", "gen_ai.operation.name": op}),
            _sp("7", "1", op, status="ok", **{"gen_ai.tool.name": "fix_5", "gen_ai.operation.name": op}),
            _sp("8", "1", op, status="ok", **{"gen_ai.tool.name": "fix_6", "gen_ai.operation.name": op}),
        ])
    ]
...
def test_cost_spike_detector_fires_on_high_cost() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", **{
            "gen_ai.agent.run_cost_total": 15.0,
        }, child_spans=[
            _sp("2", "1", op, start_time=now, end_time=now + timedelta(seconds=5),
...
def test_transient_retry_detector_fires() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", op, status="error",
                **{"gen_ai.tool.name": "flake", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": True}),
            _sp("3", "1", op, status="ok",
                **{"gen_ai.tool.name": "flake", "gen_ai.operation.name": op}),
            _sp("4", "1", op, status="error",
                **{"gen_ai.tool.name": "flake", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": True}),
            _sp("5", "1", op, status="ok",
                **{"gen_ai.tool.name": "flake", "gen_ai.operation.name": op}),
            _sp("6", "1", op, status="error",
                **{"gen_ai.tool.name": "flake", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": True}),
            _sp("7", "1", op, status="ok",
                **{"gen_ai.tool.name": "flake", "gen_ai.operation.name": op}),
        ])
    ]
...
def test_cascading_retry_detector_fires() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", op, status="error",
                **{"gen_ai.tool.name": "step_a", "gen_ai.operation.name": op,
                   "gen_ai.retry.count": 1}),
            _sp("3", "1", op, status="ok",
                **{"gen_ai.tool.name": "step_a_retry", "gen_ai.operation.name": op}),
            _sp("4", "1", op, status="error",
                **{"gen_ai.tool.name": "step_b", "gen_ai.operation.name": op,
                   "gen_ai.retry.count": 1}),
            _sp("5", "1", op, status="ok",
                **{"gen_ai.tool.name": "step_b_retry", "gen_ai.operation.name": op}),
            _sp("6", "1", op, status="error",
                **{"gen_ai.tool.name": "step_c", "gen_ai.operation.name": op,
                   "gen_ai.retry.count": 1}),
            _sp("7", "1", op, status="ok",
...
def test_systemic_retry_detector_fires() -> None:
    ...
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", op, status="error",
                **{"gen_ai.tool.name": "a", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": False}),
            _sp("3", "1", op, status="error",
                **{"gen_ai.tool.name": "b", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": False}),
            _sp("4", "1", op, status="error",
                **{"gen_ai.tool.name": "c", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": False}),
            _sp("5", "1", op, status="error",
                **{"gen_ai.tool.name": "d", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": False}),
            _sp("6", "1", op, status="error",
                **{"gen_ai.tool.name": "e", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": False}),
            _sp("7", "1", op, status="error",
                **{"gen_ai.tool.name": "f", "gen_ai.operation.name": op,
                   "gen_ai.retry.successful": False}),
        ])
    ]
    s = _summary(total_tool_calls=6, total_retries=6)
...
def test_indeterminate_detector_fires_on_empty_output() -> None:
    spans = [
        _sp("1", None, "invoke_agent", child_spans=[
            _sp("2", "1", "plan", **{"gen_ai.response.content": "Request could not be completed properly"}),
        ])
    ]
...
def test_clean_run_produces_no_critical_anomalies() -> None:
        ...
        _sp("1", None, "invoke_agent",
            ...
            **{"gen_ai.response.content": "Task completed successfully with all steps verified."},
            child_spans=[
                ...
                _sp("3", "1", op,
                    start_time=now, end_time=now + timedelta(seconds=5),
                    status="ok", **{"gen_ai.tool.name": "lookup", "gen_ai.operation.name": op}),
                _sp("4", "1", op,
                    start_time=now, end_time=now + timedelta(seconds=5),
                    status="ok", **{"gen_ai.tool.name": "verify", "gen_ai.operation.name": op}),
                _sp("5", "1", op,
                    start_time=now, end_time=now + timedelta(seconds=5),
                    status="ok", **{"gen_ai.tool.name": "report", "gen_ai.operation.name": op}),
            ]),
    ]
    ...
    assert not critical_anomalies, (
        f"Clean run should not produce critical anomalies, got: "
        f"{list(critical_anomalies.keys())}"
    )